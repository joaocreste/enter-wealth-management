 ...
 *
 * Indicators and thresholds are DATA. A desk adds "Bund de 10 anos acima de 3,20%" by
 ...
 *
 * A threshold is an opinion about a number, and the portal prints it beside an
 * observed price that names its provider. So each one carries `rationale` —
 * why this level and not another — and `source`, the document or decision that
 * set it.
 *
 * Every threshold below is `source: null` on purpose. These are desk
 * conventions written into this file, not a published XP view, and the portal
 * marks them as unsigned wherever it shows them. A desk that adopts one puts
 * its own minute in `source`; until then nobody should be able to mistake
 * "US$ 120" for a house call (§29, §30).
 */
export const TRIGGERS = [
  { id: 'trg_brent_120', label: 'Brent acima de US$ 120/bbl', indicator_key: 'brent', comparator: 'gt', threshold: 120, unit: 'USD/bbl', asset_classes: ['Commodities', 'Equities BR', 'Fixed Income'], action: 'Review energy-sensitive holdings and the inflation path assumed in the fixed-income sleeve.' , action_pt: 'Revisar as posições sensíveis a energia e a premissa de inflação da renda fixa.', rationale: 'Acima de US$ 120 o petróleo deixa de ser custo setorial e vira choque de inflação, com repasse à curva de juros local.', source: null },
  { id: 'trg_brent_90', label: 'Brent acima de US$ 90/bbl', indicator_key: 'brent', comparator: 'gt', threshold: 90, unit: 'USD/bbl', asset_classes: ['Commodities', 'Equities BR'], action: 'Discuss the inflation pass-through to Brazilian consumer names and the effect on the local rates curve.' , action_pt: 'Discutir o repasse do petróleo à inflação, o efeito nas empresas de consumo e na curva de juros local.', rationale: 'US$ 90 é o nível em que o repasse do petróleo começa a aparecer nas margens das empresas de consumo brasileiras.', source: null },
  { id: 'trg_us10y_500', label: 'Treasury de 10 anos acima de 5,00%', indicator_key: 'us10y', comparator: 'gt', threshold: 5.0, unit: '%', asset_classes: ['Fixed Income', 'Equities Global'], action: 'Reassess duration and the discount rate applied to long-duration global equity.' , action_pt: 'Reavaliar a duration da carteira e a taxa de desconto aplicada à renda variável global de duração longa.', rationale: '5,00% é a marca em que a renda variável global de duração longa costuma ser remarcada pela taxa de desconto.', source: null },
  { id: 'trg_us10y_450', label: 'Treasury de 10 anos acima de 4,50%', indicator_key: 'us10y', comparator: 'gt', threshold: 4.5, unit: '%', asset_classes: ['Fixed Income', 'Equities Global'], action: 'Confirm the portfolio duration is still inside the approved range.' , action_pt: 'Confirmar que a duration da carteira segue dentro da faixa aprovada.', rationale: '4,50% é o ponto em que a duration da carteira merece uma conferência antes da próxima reunião.', source: null },
  { id: 'trg_vix_30', label: 'VIX acima de 30 por cinco pregões', indicator_key: 'vix', comparator: 'gt', threshold: 30, unit: 'index', persistence_days: 5, asset_classes: ['Equities Global', 'Equities BR'], action: 'Sustained stress: review equity sizing before adding risk.' , action_pt: 'Estresse sustentado: revisar o tamanho da posição em renda variável antes de adicionar risco.', rationale: 'VIX acima de 30 é estresse; sustentado por cinco pregões, deixa de ser ruído e vira regime.', source: null },
  { id: 'trg_dxy_105', label: 'Índice do dólar acima de 105', indicator_key: 'dxy', comparator: 'gt', threshold: 105, unit: 'index', asset_classes: ['FX', 'Equities Global'], action: 'A strong dollar is a headwind for emerging assets; review unhedged exposure.' , action_pt: 'Dólar forte é vento contrário para ativos emergentes; revisar a exposição sem hedge.', rationale: 'Acima de 105 o dólar forte historicamente drena fluxo de mercados emergentes.', source: null },
  { id: 'trg_dxy_92', label: 'Índice do dólar abaixo de 92', indicator_key: 'dxy', comparator: 'lt', threshold: 92, unit: 'index', asset_classes: ['FX', 'Equities Global'], action: 'A weak dollar supports emerging assets; review whether unhedged USD exposure is still earning its place.' , action_pt: 'Dólar fraco favorece ativos emergentes; avaliar se a exposição em dólar sem hedge segue justificada.', rationale: 'Abaixo de 92 o dólar fraco favorece emergentes, e a exposição em dólar sem hedge perde parte da razão de ser.', source: null },
  { id: 'trg_usdbrl_600', label: 'Dólar acima de R$ 6,00', indicator_key: 'usdbrl', comparator: 'gt', threshold: 6.0, unit: 'BRL', asset_classes: ['FX', 'Equities Global'], action: 'Translate the currency move into the client result before the meeting; unhedged offshore positions gain in reais.' , action_pt: 'Traduzir o movimento cambial para o resultado do cliente antes da reunião; posições no exterior sem hedge ganham em reais.', rationale: 'A R$ 6,00 o câmbio sozinho explica boa parte do resultado de uma carteira com posição no exterior sem hedge.', source: null },
  { id: 'trg_usdbrl_500', label: 'Dólar acima de R$ 5,00', indicator_key: 'usdbrl', comparator: 'gt', threshold: 5.0, unit: 'BRL', asset_classes: ['FX', 'Equities Global'], action: 'Explain the currency contribution explicitly in the monthly letter.' , action_pt: 'Explicar a contribuição do câmbio de forma explícita na carta mensal.', rationale: 'A partir de R$ 5,00 a contribuição cambial deixa de ser detalhe e precisa ser dita na carta.', source: null },
  { id: 'trg_gold_4000', label: 'Ouro acima de US$ 4.000/oz', indicator_key: 'gold', comparator: 'gt', threshold: 4000, unit: 'USD/oz', asset_classes: ['Commodities'], action: 'Discuss whether the gold allocation remains inside the permitted range after the move.' , action_pt: 'Avaliar se a alocação em ouro segue dentro da faixa permitida após o movimento.', rationale: 'US$ 4.000/oz costuma levar a parcela em ouro para fora da faixa sem que ninguém tenha comprado nada.', source: null },
  { id: 'trg_ipca_060', label: 'IPCA mensal acima de 0,60%', indicator_key: 'ipca', comparator: 'gt', threshold: 0.60, unit: '% a.m.', asset_classes: ['Fixed Income'], action: 'Inflation surprise: review the balance between inflation-linked and CDI-linked holdings.' , action_pt: 'Surpresa inflacionária: revisar o equilíbrio entre renda fixa indexada ao IPCA e ao CDI.', rationale: '0,60% ao mês anualiza acima do teto da meta; é surpresa inflacionária, não ruído mensal.', source: null },
  { id: 'trg_selic_1500', label: 'Selic em 15,00% ou acima', indicator_key: 'selic', comparator: 'gte', threshold: 15.0, unit: '% a.a.', asset_classes: ['Fixed Income', 'Cash', 'Equities BR'], action: 'At this level of carry, confirm the client understands the opportunity cost of equity risk.' , action_pt: 'Com esse nível de carrego, confirmar que o cliente entende o custo de oportunidade do risco em renda variável.', rationale: 'Com a Selic em 15% o carrego do caixa compete de frente com o prêmio de risco da bolsa.', source: null },
  { id: 'trg_hy_stress', label: 'ETF de high yield dos EUA em queda de mais de 4% no mês', indicator_key: 'hy_etf', comparator: 'lt', threshold: -0.04, unit: 'mtd', asset_classes: ['Fixed Income'], action: 'Credit stress: review private-credit fund exposure and redemption terms.' , action_pt: 'Estresse de crédito: revisar a exposição a fundos de crédito privado e as janelas de resgate.', rationale: 'Queda de mais de 4% no mês no high yield americano é o primeiro sinal de estresse de crédito que chega aos fundos locais.', source: null },
  { id: 'trg_btc_100k', label: 'Bitcoin acima de US$ 100.000', indicator_key: 'btc', comparator: 'gt', threshold: 100000, unit: 'USD', asset_classes: ['Digital Assets'], action: 'Rebalance digital-asset sleeves back inside the permitted range where one exists.' , action_pt: 'Rebalancear a parcela em ativos digitais de volta para dentro da faixa permitida, onde ela existir.', rationale: 'US$ 100 mil é um número redondo e nada além disso; serve como marca de rebalanceamento da parcela em ativos digitais, onde ela existe.', source: null },
];
