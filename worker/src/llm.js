 ...
 * world, the events as the desk phrased them, the month's figures for the
 * comment, a reading of the risk-and-return chart, the points as the analysis
 * phrased them, and a closing with the next meeting.
 */
export function deterministicReportNarrative(facts) {
  ...
  const b = f.world?.briefing || {};
  const held = new Set((f.allocation || []).map((a) => a.asset_class));
  const summary = two(f.world?.summary);
  ...
  const worldParts = [
    f.world?.headline ? `${String(f.world.headline).replace(/[.!?]\s*$/, '')}.` : null,
    summary,
    risk && !said(risk, summary) ? risk : null,
  ];
  if (!worldParts.some(Boolean)) {
    if (held.has('Equities BR') || held.has('Equities Global')) worldParts.push(two(b.equities));
    if (held.has('Fixed Income') || held.has('Cash')) worldParts.push(two(b.rates_credit));
  }
  const world = worldParts.filter(Boolean).join(' ') || 'Não há panorama de mercado disponível para a data deste relatório.';
  const events = (f.world?.events || []).map((e) => ({ title: e.title, text: two(e.why) }));
  ...
  if (p.monthly_return_label == null) perf.push(`Não foi possível apurar a rentabilidade consolidada de ${month} com os dados disponíveis.`);
    ...
    if (p.worst) perf.push(`Começo pelo que pesou: ${p.worst.name} foi o que mais tirou do resultado em ${month}, ${p.worst.contribution_label} sobre a carteira.`);
    if (p.best) perf.push(`Do lado bom, ${p.best.name} somou ${p.best.contribution_label}.`);
    perf.push(`No total, a sua carteira fez ${p.monthly_return_label} no mês${p.benchmark_label ? `, contra ${p.benchmark_label} da carteira de referência` : ''}${p.ytd_label ? `; no ano, ${p.ytd_label}` : ''}.`);
  }

  const a12 = f.assets_12m || {};
  const assetParts = [];
  if (a12.best_for_risk) assetParts.push(`Nos últimos doze meses, ${a12.best_for_risk} foi o seu ativo que melhor pagou pelo risco que correu.`);
  if (a12.lowest_return && a12.lowest_return !== a12.best_for_risk) assetParts.push(`${a12.lowest_return} foi o que menos rendeu no período.`);
  const cdi = (a12.references || []).find((r) => /CDI/i.test(r.name));
  if (cdi) assetParts.push(`O CDI, a referência sem risco, rendeu ${cdi.return_label}.`);
  const assets_comment = assetParts.join(' ');

  const improve = (f.improve_points || []).map((d) => ({ title: d.title, text: d.text }));
  const worsen = (f.worsen_points || []).map((d) => ({ title: d.title, text: d.text }));

  const off = (f.allocation || []).filter((x) => x.position_label !== 'no alvo');
  const outside = (f.allocation || []).filter((x) => x.outside_band);
  const allocation_comment = outside.length
    ? `${outside.map((x) => x.label).join(', ')} ${outside.length === 1 ? 'está fora da faixa' : 'estão fora das faixas'} combinada${outside.length === 1 ? '' : 's'} na sua política; ${off.length} ${off.length === 1 ? 'classe está' : 'classes estão'} longe do alvo.`
    : off.length
      ? `Todas as classes estão dentro das faixas da sua política; ${off.length} ${off.length === 1 ? 'está' : 'estão'} longe do alvo.`
      : 'Todas as classes estão dentro das faixas e perto do alvo da sua política.';

  const closing = f.next_meeting
    ? `Nossa próxima reunião está marcada para ${f.next_meeting}. Levo estes pontos para conversarmos. Se preferir falar antes, é só me chamar.`
    : 'Se quiser conversar sobre qualquer ponto deste relatório, é só me chamar.';

  return {
    greeting: `Prezado ${first},`,
    opening: `Este é o seu relatório de ${month}: o que está acontecendo no mundo, como a sua carteira andou e o que vale conversarmos.`,
    world, events, performance_comment: perf.join(' '), assets_comment, improve, worsen, allocation_comment, closing, sign_off: 'Um abraço,', language: 'pt-BR',
  };
}
